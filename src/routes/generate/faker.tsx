...
const DATA_TYPES: Record<Category, DataTypeConfig[]> = {
  ...
  location: [
    ...
    {
      generate: () => faker.location.longitude().toString(),
      label: 'Longitude',
      value: 'longitude',
    },
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="w-[100px]">
            <NumberInput
              ...
              onValueChange={v =>
                onOptionsChange({ ...options, lorem: { ...options.lorem, wordCount: v ?? 3 } })
              }
              min={1}
              max={100}
              stepper={1}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="w-[100px]">
            <NumberInput
              ...
              onValueChange={v =>
                onOptionsChange({ ...options, lorem: { ...options.lorem, sentenceCount: v ?? 3 } })
              }
              min={1}
              max={20}
              stepper={1}
          ...
          <div className="w-[100px]">
            <NumberInput
              ...
              onValueChange={v =>
                onOptionsChange({
                  ...options,
                  lorem: { ...options.lorem, paragraphSentences: v ?? 3 },
                })
              }
              min={1}
              max={20}
      ...
      count:
        typeof search.count === 'number' && search.count >= 1 && search.count <= 100
          ...
          : typeof search.count === 'string' && !isNaN(Number(search.count))
            ...
            : undefined,
      paragraphSentences:
        typeof search.paragraphSentences === 'number' &&
        search.paragraphSentences >= 1 &&
        search.paragraphSentences <= 20
          ? search.paragraphSentences
          : typeof search.paragraphSentences === 'string' &&
              !isNaN(Number(search.paragraphSentences))
            ? Math.min(20, Math.max(1, Number(search.paragraphSentences)))
            : undefined,
      sentenceCount:
        typeof search.sentenceCount === 'number' &&
        search.sentenceCount >= 1 &&
        search.sentenceCount <= 20
